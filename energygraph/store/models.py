...
def findSensorMetaByID(sensorID, callFrom=None):
    ...
    logger.debug( 'key5-findSensorMetaByID:%d/%d'%(epoch,sensorID) ) 
    id = memcache.get( 'key5-findSensorMetaByID:%d/%d'%(epoch,sensorID) ) 
    if id != None:
        id = json.loads( id )
    ...
    memcache.put( 'key5-findSensorMetaByID:%d/%d'%(epoch,sensorID), json.dumps( ret ) , 24*3600 )
    return ret
...
def findGroupTotalSensorID( groupID , ignoreList ):
    ...
    logger.debug( 'key5-findGroupTotalSensorID:%d/%d'%(groupID,epoch) ) 
    id = memcache.get( 'key5-findGroupTotalSensorID:%d/%d'%(groupID,epoch) ) 
    if id != None:
        id = long(id)
        return id
    ...
    memcache.put( 'key5-findGroupTotalSensorID:%d/%d'%(groupID,epoch), ret , 24*3600 )
    return ret
...
def findAllSensorIDsInGroup( groupID , calledBy="" ):
    ...
    logger.debug( 'key5-findAllSensorIDsInGroup:%d/%d'%(epoch,groupID) )
    id = memcache.get( 'key5-findAllSensorIDsInGroup:%d/%d'%(epoch,groupID) )
    if id != None:
        id = json.loads( id )
        return id
    ...
    memcache.put( 'key5-findAllSensorIDsInGroup:%d/%d'%(epoch,groupID), json.dumps( ret ), 24*3600 )
...
def findAllSensorsIDsByUserID( userID ):
    ...
    logger.debug( 'key5-findAllSensorsIDsByUserID:%d/%d'%(epoch,userID) )
    id = memcache.get( 'key5-findAllSensorsIDsByUserID:%d/%d'%(epoch,userID) )
    if id != None:
        id = json.loads( id )
        return id
    ...
    memcache.put( 'key5-findAllSensorsIDsByUserID:%d/%d'%(epoch,userID), json.dumps(ret), 24*3600 )
    return ret
...
def getSensorIDByName( sensorName ):
    ...
    logger.debug( 'key5-getSensorIDByName:%s'%(sensorName) )
    id = memcache.get( 'key5-getSensorIDByName:%s'%(sensorName) )
    if id != None:
        id = long( id )
        if id != 0:
    ...
    if sensor != None:
        ret = sensor.sensorID
        memcache.put( 'key5-getSensorIDByName:%s'%(sensorName) , ret, 24*3600 )
...
def findSensorID( userName, sensorName, create=False , createGroup=False ):
    ...
    logger.debug( 'key5-sensorID:%s/%s'%(userName,sensorName) )
    id = memcache.get( 'key5-sensorID:%s/%s'%(userName,sensorName) )
    if id != None:
        id = long( id )
        return id
    ...
    if sensor is not None:
        # put in memcache 
        memcache.put(  'key5-sensorID:%s/%s'%(userName,sensorName), str(sensor.sensorID) , 3600 )
...
def getUserPasswordHash( userName ):
    ...
    assert epoch > 0 
    logger.debug( 'key5-getUserPasswordHash:%d/%s'%(epoch,userName)  )
    id = memcache.get( 'key5-getUserPasswordHash:%d/%s'%(epoch,userName)  )
    if id != None:
        id = str( id )
        return id
    ...
    memcache.put( 'key5-getUserPasswordHash:%d/%s'%(epoch,userName) ,ret , 24*3600 )
    return ret
...
def getUserMetaByUserID( userID ):
    ...
    logger.debug( 'key5-getUserMetaByUserID:%s/%s'%(epoch,userID) ) 
    id = memcache.get( 'key5-getUserMetaByUserID:%d/%d'%(epoch,userID) ) 
    if id != None:
    ...
    memcache.put( 'key5-getUserMetaByUserID:%d/%d'%(epoch,userID), j , 24*3600 )
    return ret
...
def updateUserSettingsEpoch(userName):
    ...
    memcache.put( 'key5-UserSettingEpoch:%s'%(user.userName) , user.settingEpoch , 24*3600 )
    memcache.put( 'key5-getUserSettingEpochByUserID:%s'%(user.userID) , user.settingEpoch , 24*3600 )
...
def getUserSettingEpoch(userName):
    ...
    logger.debug( 'key5-UserSettingEpoch:%s'%(userName)  )
    id = memcache.get( 'key5-UserSettingEpoch:%s'%(userName)  )
    if id != None:
        ret = long(id)
    ...
    memcache.put( 'key5-UserSettingEpoch:%s'%(userName) , ret , 24*3600 )
...
def getUserSettingEpochByUserID(userID):
    logger.debug( 'key5-getUserSettingEpochByUserID:%s'%(userID)  )
    id = memcache.get( 'key5-getUserSettingEpochByUserID:%s'%(userID)  )
    if id != None:
        return long(id)
    ...
    if user.settingEpoch is None:
        ...
        ret = user.settingEpoch
    memcache.put( 'key5-getUserSettingEpochByUserID:%s'%(userID) , ret , 24*3600 )
    return ret
...
def findUserNameBySensorID(sensorID):
    ...
    logger.debug( 'key5-findUserNameBySensorID:%d'%(sensorID)  )
    id = memcache.get( 'key5-findUserNameBySensorID:%d'%(sensorID)  )
    if id != None:
        ret = str(id)
    ...
    memcache.put( 'key5-findUserNameBySensorID:%d'%(sensorID) , ret , 24*3600 )
    globalUserNameBySensorID[sensorID] = ret
...
def findUserIDByName( userName ): 
    ...
    id = None
    logger.debug( 'key5-userID:%s'%(userName) )
    id = memcache.get( 'key5-userID:%s'%(userName) )
    ...
    if user != None:
        ...
        # put in memcache 
        memcache.put( 'key5-userID:%s'%(userName) , str(id) , 24*3600 )
        return id
...
def findUserNameByID( userID ):
    logger.debug( 'key5-findUserNameByID:%d'%userID )
    val = memcache.get( 'key5-findUserNameByID:%d'%userID )
    if val != None:
        val = str( val )
        return val
    ...
    memcache.put( 'key5-findUserNameByID:%d'%userID, ret, 24*3600 )
    return ret


def findUserIdByName( userName ):
    logger.debug( 'key5-findUserIdByName:%s'%userName )
    val = memcache.get( 'key5-findUserIdByName:%s'%userName )
    if val != None:
        val = long( val )
        return val
    ...
    memcache.put( 'key5-findUserIdByName:%s'%userName, ret, 24*3600 )
    return ret
...
def hourlyPatchFast(): 
    ...
    #cursor = memcache.get('key5-hourly_path_cursor_%d_a'%level)
    #if ( cursor ):
    ...
    #cursor = query.cursor()
    #memcache.put('key5-hourly_path_cursor_%d_a'%level,cursor, 2*24*3600 )
...
def getHourlyEnergyTotalByGroupID( groupID, utime ):
    ...
    logger.debug( 'key5-getHourlyEnergyTotalByGroupID:%d/%d'%(groupID,utime) )
    val = memcache.get( 'key5-getHourlyEnergyTotalByGroupID:%d/%d'%(groupID,utime) )
    if val != None:
        val = float( val )
        return val
    ...
    assert ret >= 0.0
    memcache.put(  'key5-getHourlyEnergyTotalByGroupID:%d/%d'%(groupID,utime), ret, 5*60 )
    return ret
...
def getHourlyEnergyOtherByGroupID( groupID, utime ):
    ...
    logger.debug( 'key5-getHourlyEnergyOtherByGroupID:%d/%d'%(groupID,utime) )
    val = memcache.get( 'key5-getHourlyEnergyOtherByGroupID:%d/%d'%(groupID,utime) )
    if val != None:
        val = float( val )
        return val
    ...
    assert ret >= 0.0
    memcache.put(  'key5-getHourlyEnergyOtherByGroupID:%d/%d'%(groupID,utime), ret, 5*60 )
    return ret
...
def getHourlyEnergyBySensorID( sensorID, utime ): # TODO - can we get rid of this 
    ...
    logger.debug( 'key5-getHourlyEnergyBySensorID:%d/%d'%(sensorID,utime) )
    val = memcache.get( 'key5-getHourlyEnergyBySensorID:%d/%d'%(sensorID,utime) )
    if val != None:
        val = float( val )
        return val
    ...
    memcache.put(  'key5-getHourlyEnergyBySensorID:%d/%d'%(sensorID,utime), hourly.energy, 24*3600 )
...
def getHourlyIntegralBySensorID( sensorID, utime ):
    ...
    logger.debug( 'key5-getHourlyIntegralBySensorID:%d/%d'%(sensorID,utime) )
    val = memcache.get( 'key5-getHourlyIntegralBySensorID:%d/%d'%(sensorID,utime) )
    if val != None:
        val = float( val )
        return val
    ...
    memcache.put(  'key5-getHourlyIntegralBySensorID:%d/%d'%(sensorID,utime), hourly.integral, 24*3600 )
