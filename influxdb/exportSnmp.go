...
./senmlCat -ijsonl -topic snmp -linp -post "http://10.1.3.254:8086/write?db=junk3" dump2016-06-02-2000.jsonl
...
func dumpRange(databaseUrl string, start time.Time, end time.Time, seriesName string, fileName string) error {
	...
	sql := "SELECT time,column,host,value FROM " + seriesName + " WHERE time >= " + strconv.FormatInt(start.UnixNano(), 10) + " AND time < " + strconv.FormatInt(end.UnixNano(), 10)
	//fmt.Println("sql=", sql)
		...
		for _, series := range response.Results[0].Series {
			...
			for _, point := range series.Values {
				...
				case string:
					column = string(x)
				default:
					fmt.Println("unknown  type for column ", x)
				}
				...
				case string:
					host = string(x)
				default:
					fmt.Println("unknown  type for host ", x)
				}
	...
	options := senml.OutputOptions{Topic: "snmp"}
